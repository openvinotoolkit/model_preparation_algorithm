        ...
        def _get_warmup_lr(cur_iters, regular_lr):
            if self.warmup == 'constant':
                ...
                k = (1 - cur_iters / self.warmup_iters) * (1 -
                                                           self.warmup_ratio)
                print(f"regular_lr : {regular_lr}, warmup factor : {1-k}")
...
@HOOKS.register_module()
class ReduceLROnPlateauLrUpdaterHook(CustomLrUpdaterHook):
    """
    ...
    init_value_map = {'greater': -inf, 'less': inf}
    greater_keys = [
        'accuracy_top-1', 'acc', 'top', 'AR@', 'auc', 'precision', 'mAP', 'mDice', 'mIoU',
        'mAcc', 'aAcc'
    ]
    less_keys = ['loss']
    ...
    def get_lr(self, runner: BaseRunner, base_lr: float):
        ...
        if hasattr(runner, self.metric):
            ...
            print(f"runner has attr {self.metric} {score}")
