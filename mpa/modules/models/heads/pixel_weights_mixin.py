...
class PixelWeightsMixin(nn.Module):
    ...
    def forward_train(
        ...
        if return_logits:
            logits = (
                self.forward_output if self.forward_output is not None else seg_logits
            )
    ...
    def losses(self, seg_logit, seg_label, train_cfg, pixel_weights=None):
        ...
        for loss_idx, loss_module in enumerate(losses_decode):
            loss_value, loss_meta = loss_module(
                seg_logit, seg_label, pixel_weights=pixel_weights
            )
...
class PixelWeightsMixin2(PixelWeightsMixin):
    ...
    def losses(
        ...
        for loss_idx, loss_module in enumerate(losses_decode):
            loss_value, loss_meta = loss_module(
                seg_logit, seg_label, valid_label_mask, pixel_weights=pixel_weights
            )
