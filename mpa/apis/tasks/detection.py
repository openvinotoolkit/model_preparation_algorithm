...
from mpa.apis.configs import DetectionConfig, TrainType
from mpa.apis.tasks.base import BaseTask
from mpa.utils.config_utils import MPAConfig
from mpa.utils.logger import get_logger
from mmcv.utils import ConfigDict
from mmdet.apis.ote.apis.detection.config_utils import remove_from_config
...
class DetectionInferenceTask(BaseTask, IInferenceTask, IExportTask, IEvaluationTask, IUnload):
    ...
    def _init_recipe(self):
        ...
        elif train_type == TrainType.DynamicLabels:
            raise NotImplementedError(f'train type {train_type} is not implemented yet.')
        else:
            raise NotImplementedError(f'train type {train_type} is not implemented yet.')
    ...
    def _init_test_data_cfg(self, dataset: DatasetEntity):
        data_cfg = ConfigDict(
            data=ConfigDict(
                train=ConfigDict(
                    _classes=list(label.name for label in self._labels),  # Just for task adaptation info
                ),
                test=ConfigDict(
                    ote_dataset=dataset,
                    labels=self._labels,
        ...
        data_cfg = ConfigDict(
            data=ConfigDict(
                train=ConfigDict(
                    ...
                    labels=self._labels,
                    _classes=list(label.name for label in self._labels),
                ),
                val=ConfigDict(
