...
def topk_symbolic(g, self, k, dim, largest, sorted, out=None):
    ...
    top_values, top_indices = sym_help._topk_helper(
        g, self, k, dim, largest, sorted, out
    )
...
def multiclass_nms_core_symbolic(
    g, multi_bboxes, multi_scores, score_thr, nms_cfg, max_num=-1
):
    ...
    # Select bboxes.
    out_bboxes = reshape(
        g, g.op("Gather", multi_bboxes_flat, flat_box_indices, axis_i=0), [-1, 4]
    )
    out_scores = reshape(
        g, g.op("Gather", multi_scores_flat, flat_score_indices, axis_i=0), [-1, 1]
    )
    # Having either batch size or number of classes here equal to one is the limitation of implementation.
    class_indices = reshape(
        g, cast(add(class_indices, batch_indices), "Float"), [-1, 1]
    )
    ...
    elements_num = sym_help._size_helper(
        g, out_scores, dim=g.op("Constant", value_t=torch.LongTensor([0]))
    )
...
def roi_feature_extractor_symbolics(
    g, rois, *feats, output_size=1, featmap_strides=1, sample_num=1
):
...
def patch_nms_aten_to():
    ...
    if version.parse(torch.__version__) < version.parse("1.7.1"):
        ...
        def forward(
            ctx, bboxes, scores, iou_threshold, score_threshold, max_num, offset
        ):
            with torch.jit._disable_tracing():
                inds = original_forward(
                    ctx, bboxes, scores, iou_threshold, score_threshold, max_num, offset
                )
            return inds
...
def nms_symbolic_with_score_thr(
    g, bboxes, scores, iou_threshold, score_threshold, max_num, offset
):
    """
